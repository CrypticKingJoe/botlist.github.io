...
route.post("/", async (req, res, next) => {
    ...
    let memberCheck = req.app.get('client').guilds.cache.get(discord.guild_id).member(user.id);
    ...
    try {
        let r = req.app.get('client').guilds.cache.get(discord.guild_id).roles.cache.find(r => r.id === discord.ADMIN_USER);
        await r.setMentionable(true)
        await req.app.get('client').channels.cache.find(c => c.id === discord.mod_log_id).send(`<@${owners[0]}> added <@${bot.id}>: ${r}`);
